...
decades <- unique(tb$decade)

# UI
ui <- fluidPage(
   
   # Application title
   titlePanel("01-cordex-decades"),
   ...
   sidebarLayout(
      sidebarPanel(
         ...
         #             label = "City:", 
         #             multiple = TRUE, 
         #             choices = c("Athens", "Berlin", "Brussels")),
         # 
...
server <- function(input, output) {
  ...
  # If a city appears in the seelected list of cities, calculate new data serie. Otherwise, null.  
  # tb_athens <- reactive({
  #   if (("Athens" %in% cities()) | (length(cities())==0)) {
  #     tb %>% 
  ...
  selected_years_to_print <- reactive({
    if(input$years[1] == input$years[2]) { 
      ...
      paste(input$years[1], "-" , input$years[1]+9)
    } else {
      paste("Between ",input$years[1], " - " , input$years[1]+9, " and ", input$years[2], "-", input$years[2]+9)
    }
  })
  ...
  output$hcontainer <- renderHighchart({
    ...
    hc <- hc %>% 
      ...
      hc_subtitle(text = paste("Decade median values according to model EUR-11_ICHEC-EC-EARTH_rcp85_r1i1p1_KNMI-RACMO22E for major european cities - ",
